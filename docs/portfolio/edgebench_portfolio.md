...
- Jetson `resnet18` same-precision compare에서 mean latency `2.8647ms → 2.8265ms`, p99 `3.1388ms → 3.0620ms` 변화가 관찰되었고, 현재 threshold 기준으로는 실제 **neutral** 로 판정되는 것을 확인
- Jetson `yolov8n` same-precision compare에서 mean latency `14.4592ms → 14.1108ms`, p99 `15.4154ms → 15.2565ms` 결과가 실제 **neutral** 로 판정되는 것을 확인
...
- Jetson TensorRT (`resnet18`, fp16, batch1, 224x224)
  - mean latency: 2.8647ms → 2.8265ms
  - p99 latency: 3.1388ms → 3.0620ms
  - overall: neutral

- Jetson TensorRT (`yolov8n`, fp16, batch1, 640x640)
  - mean latency: 14.4592ms → 14.1108ms
  - p99 latency: 15.4154ms → 15.2565ms
  - overall: neutral
...
- 즉, EdgeBench는 cross-precision trade-off 분석뿐 아니라, Jetson 실기 환경에서의 same-precision regression / neutral tracking과 threshold 기반 판정 구조까지 실제 검증된 형태로 확장되었습니다.
