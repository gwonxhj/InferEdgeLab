...
- Jetson `resnet18` same-precision compare에서 mean latency `3.4644ms → 3.6311ms`, p99 `3.5057ms → 3.7170ms` 변화가 실제 regression으로 판정되는 것을 확인
...
- 즉, EdgeBench는 cross-precision trade-off 분석뿐 아니라, Jetson 실기 환경에서의 same-precision regression tracking까지 검증된 구조로 확장되었습니다.
