...
> EdgeBench is a CLI-first inference validation framework with reusable service and FastAPI adapter layers for edge benchmarking workflows.
...
- FastAPI-based read-only API adapter for compare / history-report / summarize / list-results
- `edgebench serve` command for launching the API server from the CLI
- CI-ready validation workflow for benchmark gating
- `enrich-result` / `enrich-pair` 기반 post-profile accuracy attachment workflow
...
- **향후 서비스화에 대비하기 위해**: compare / history / summarize / list 흐름을 service layer와 FastAPI adapter로 재사용 가능하게 정리
...
- `enrich-result`로 runtime structured result에 detection accuracy 수동 부착 검증 완료
- `enrich-pair`로 fp16 / int8 runtime result pair에 accuracy를 일괄 부착한 뒤 `acceptable_tradeoff`까지 재현 검증 완료
...
- Runtime results can be enriched with external detection accuracy metadata after profiling
- Enriched RKNN runtime pairs can be compared with `map50` as the primary metric and produce accuracy-aware trade-off judgement
...
|---|---:|---:|---|
| Base | 2.8647 ms | 3.1388 ms | 기준 측정 |
| New | 2.8265 ms | 3.0620 ms | same-precision neutral 판정 |

**Example 2 (Jetson TensorRT, yolov8n, same-precision)**

| Run | Mean Latency | P99 Latency | Interpretation |
|---|---:|---:|---|
| Base | 14.4592 ms | 15.4154 ms | 기준 측정 |
| New | 14.1108 ms | 15.2565 ms | same-precision neutral 판정 |
...
**Example 4 (YOLOv8n, Odroid M2, RKNN runtime cross-precision + enriched accuracy)**

| Precision | Mean Latency | Accuracy (mAP50) | Interpretation |
|---|---:|---:|---|
| FP16 | 71.8846 ms | 0.7791 | 기준 결과 |
| INT8 | 35.0657 ms | 0.7977 | `tradeoff_faster`, `acceptable_tradeoff` |

이 예시는 runtime profiling 결과 자체에는 accuracy가 없더라도, 이후 `enrich-result` 또는 `enrich-pair`로 detection metric을 structured result에 부착하면 curated import와 동일한 accuracy-aware compare 흐름으로 다시 연결할 수 있음을 보여줍니다. 즉, EdgeBench는 “runtime latency evidence”와 “accuracy evidence”를 분리 수집한 뒤 같은 schema로 재결합하는 validation workflow까지 지원합니다.
...
- 🎯 Accuracy Evaluation / Attachment
  - classification manifest 기반 top-1 accuracy 평가
  - accuracy 결과를 structured result에 함께 저장
  - `enrich-result`로 기존 structured result에 외부 accuracy JSON 부착
  - `enrich-pair`로 base/new result pair에 accuracy를 일괄 부착
...
> 참고: 현재 EdgeBench는 ONNX Runtime CPU와 TensorRT Jetson 경로를 직접 실행(profiling)하며,
> RKNN은 (1) curated result import 경로와 (2) Odroid M2 실기 runtime profiling 경로를 함께 지원합니다.
> 또한 runtime 결과에 대해서는 `enrich-result` / `enrich-pair`를 통해 외부 accuracy evidence를 후처리로 연결할 수 있습니다.
...
현재 구조는 아래 4개 계층으로 정리되어 있습니다.

- **CLI Layer**
  - `analyze`, `profile`, `evaluate`, `compare`, `compare-latest`
  - `list-results`, `history-report`, `summarize`
  - `enrich-result`, `enrich-pair`
  - `serve`
- **Service Layer**
  - compare / history-report / summarize / list-results 도메인 로직 재사용
- **API Adapter Layer**
  - FastAPI 기반 read-only endpoint (`/health`, `/api/list-results`, `/api/summarize`, `/api/history-report`, `/api/compare`)
- **Engine Layer**
  - ONNX Runtime CPU
  - TensorRT Jetson GPU first working path
  - RKNN curated import + Odroid runtime validation path
...
이 한계는 `enrich-result` / `enrich-pair`로 보완할 수 있습니다. 즉, runtime profiling으로 생성한 fp16 / int8 structured result에 외부 detection accuracy JSON을 나중에 부착한 뒤, 다시 `compare` 또는 `compare-latest`를 수행하면 `map50` 기준 accuracy-aware trade-off 판정까지 연결할 수 있습니다.

예시:

```bash
poetry run edgebench enrich-pair \
  --base-result results/yolov8n.onnx__rknn__npu__fp16__b1__h640w640__20260416-071034.json \
  --base-accuracy-json tmp_accuracy_fp16.json \
  --new-result results/yolov8n.onnx__rknn__npu__int8__b1__h640w640__20260416-073138.json \
  --new-accuracy-json tmp_accuracy_int8.json \
...
poetry run edgebench compare-latest \
  ...
  --device npu \
  --selection-mode cross_precision \
  --pattern 'results_enriched/*.json'
  ```

이 workflow를 통해 runtime cross-precision pair도 unknown_risk에서 끝나지 않고, accuracy evidence가 포함된 acceptable_tradeoff 또는 caution/risky/severe_tradeoff 해석으로 확장될 수 있습니다.
...
최신 실기 검증 기준으로,
- `resnet18`은 mean latency `2.8647ms → 2.8265ms`, p99 `3.1388ms → 3.0620ms` 변화가 관찰되었지만, 현재 threshold 기준으로는 same-precision compare에서 **neutral**
- `yolov8n`은 mean latency `14.4592ms → 14.1108ms`, p99 `15.4154ms → 15.2565ms` 변화가 관찰되었고, same-precision compare에서 **neutral**
로 판정되었습니다.
...
### Auto-Synced Jetson Validation Evidence

<!-- EDGE_BENCH_JETSON_EVIDENCE:START -->
...
<!-- EDGE_BENCH_JETSON_EVIDENCE:END -->
...
- 실제 Jetson 실기 검증에서는 TensorRT가 `Using an engine plan file across different models of devices is not recommended` 경고를 출력했습니다.
- 또한 ONNX Runtime 쪽에서는 `device_discovery.cc` 관련 warning이 함께 출력될 수 있었지만, 이번 검증에서는 TensorRT profiling / `compare-latest` / Markdown / HTML report 흐름을 깨는 치명적 오류로 이어지지는 않았습니다.

### RKNN Support

- RKNN은 현재 세 가지 흐름으로 연결됩니다.
  ...
  - runtime profiling 후 `enrich-result` / `enrich-pair`로 외부 accuracy JSON을 부착하는 post-profile enrichment path
- 즉, RKNN runtime 경로는 아직 accuracy를 자동 측정하지는 않지만, 후처리 enrichment를 통해 accuracy-aware compare 흐름으로 연결할 수 있습니다.
- 현재 문서화된 RKNN accuracy-aware 예시는 curated import 결과와 enriched runtime pair 예시를 함께 기준으로 유지합니다.
...
이후 같은 결과 흐름을 HTTP API로 재사용하고 싶다면 아래처럼 API 서버를 실행할 수 있습니다.
...
## 🧩 Accuracy Attachment Demo

runtime profiling 결과에는 latency와 provenance가 잘 남지만, 실제 accuracy metric은 별도 실험 결과에서 나오는 경우가 많습니다.
이럴 때 EdgeBench는 `enrich-result` 와 `enrich-pair` 로 accuracy evidence를 후처리로 붙일 수 있습니다.

### 1. 단일 result에 accuracy 부착

```bash
poetry run edgebench enrich-result \
  results/result_a.json \
  --accuracy-json accuracy_a.json \
  --out-dir results_enriched
```
...
### 2. result pair에 accuracy 일괄 부착

```bash
poetry run edgebench enrich-pair \
  --base-result results/fp16_result.json \
  --base-accuracy-json tmp_accuracy_fp16.json \
  --new-result results/int8_result.json \
  --new-accuracy-json tmp_accuracy_int8.json \
  --out-dir results_enriched
```

### 3. accuracy-aware cross-precision compare
...
> 이 흐름을 통해 EdgeBench는 runtime-only 결과를 나중에 accuracy-aware validation evidence로 확장될 수 있습니다.
...
> 아래 marker block은 `scripts/update_benchmarks.py` 또는 `scripts/update_readme.py` 실행 시 자동 갱신됩니다.
> source는 `BENCHMARKS.md` 의 auto-generated benchmark summary marker block입니다.
...
> `BENCHMARKS.md`에는 위 auto-generated benchmark summary와 별도로,
> Jetson TensorRT validation reference, RKNN runtime validation reference, curated hardware validation 표가 수동 evidence로 함께 유지됩니다.
