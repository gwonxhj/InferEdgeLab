...
- 현재 TensorRT backend는 Jetson 환경에서 실제 실행 가능한 초기 working path까지 연결되어 있습니다.
- 현재 구현 범위에는 TensorRT engine deserialize, IO metadata 추출, CUDA device buffer allocation, dummy input generation, TensorRT execution, CLI profile 연동, structured result 저장이 포함됩니다.
- 실행 환경은 Jetson + TensorRT Python bindings + `cuda-python` low-level driver bindings를 전제로 합니다.
- macOS 또는 non-Jetson 환경에서는 TensorRT backend를 실행할 수 없습니다.
- 현재 경로는 실사용 검증이 가능한 experimental first working path이며, production-grade runtime polish 단계는 아직 남아 있습니다.

`model_path` vs `engine_path`:

- `model_path`: static analysis, reporting, result provenance에 사용하는 원본 ONNX source path
- `engine_path`: Jetson에서 runtime execution에 사용할 compiled TensorRT engine artifact path

현재 Jetson execution path:

- `model_path` 기준의 ONNX analysis와 report 생성을 유지합니다.
- `engine_path` 에서 compiled TensorRT engine load 및 deserialize를 수행합니다.
- Jetson에서 execution context, IO metadata, CUDA buffer, CUDA stream을 준비합니다.
- 기존 result/report 구조를 유지하면서 TensorRT inference와 structured result 저장을 수행합니다.

### Jetson TensorRT 환경 점검 스크립트

Jetson bring-up을 시작하기 전에 `scripts/check_jetson_tensorrt_env.py` 로 기본 환경을 먼저 확인하는 것을 권장합니다.
이 스크립트는 TensorRT runtime execution을 수행하지 않고, 사전 준비 상태만 점검합니다.

확인 항목:

- Python / platform 정보
- `tensorrt`, `onnxruntime`, `numpy` import 가능 여부
- `/etc/nv_tegra_release` 존재 여부
- 전달한 `model_path`, `engine_path` 경로 존재 여부

사용 예시:

```bash
poetry run python scripts/check_jetson_tensorrt_env.py \
  --model-path models/toy224.onnx \
  --engine-path build/toy224.engine
```

스크립트 종료 코드:

- 기본 bring-up prerequisites가 충족되면 `0`
- 필수 항목이 누락되면 `1`

### Jetson TensorRT 권장 bring-up 순서

현재 TensorRT runtime을 안정적으로 검증할 때는 아래 순서가 권장됩니다.

1. `scripts/check_jetson_tensorrt_env.py` 로 Jetson 환경과 기본 의존성을 먼저 확인합니다.
2. 작은 ONNX 모델과 대응되는 `.engine` artifact를 준비합니다.
3. `profile --engine tensorrt --engine-path ...` 로 실제 deserialize, metadata 추출, buffer allocation, execution 경로를 검증합니다.
4. 먼저 `resnet18` 같은 classification 모델로 fp16 profiling 결과와 structured result 저장을 확인합니다.
5. 그 다음 `yolov8n` 으로 larger detector 경로를 검증하고 compare/report 흐름까지 연결합니다.

향후 확장 예정:
- TensorRT runtime 고도화
- RKNN
- Jetson CUDA Backend
- C++ 추론 엔진
...
Jetson에서 TensorRT runtime first-run을 검증할 때는 아래 순서가 안전합니다.

1. Jetson 환경에서 `python -c "import tensorrt"` 가 정상 동작하는지 확인합니다.
2. `python -c "from cuda.bindings import driver as cuda"` 가 정상 동작하는지 확인합니다.
3. ONNX 원본 모델과 대응되는 `.engine` artifact가 모두 준비되었는지 확인합니다.
4. `resnet18` 기준으로 `poetry run edgebench profile ... --engine tensorrt --engine-path ... --precision fp16` 를 실행합니다.
5. engine deserialize, metadata 추출, dummy input 생성, execution, structured result 저장이 모두 통과하는지 확인합니다.
6. 같은 방식으로 `yolov8n` 경로를 검증합니다.
7. 마지막으로 `compare-latest` 로 TensorRT GPU 결과를 직접 비교해 result metadata와 compare workflow를 확인합니다.
...
- 결과 JSON에는 `engine=tensorrt`, `device=gpu`, `precision=fp16` metadata가 구조적으로 저장되어야 합니다.
- 현재 구현은 Jetson 실기 bring-up과 validation에 적합한 초기 working runtime이며, 장기적으로는 stream 관리, lifecycle, optimization 고도화 여지가 있습니다.
...
- 먼저 환경 점검 스크립트로 Jetson / TensorRT 사전 준비 상태를 확인합니다.
- 그 다음 `edgebench profile ... --engine tensorrt --engine-path ...` 순서로 실제 profiling을 수행합니다.
...
- non-Jetson 또는 TensorRT / cuda-python 미설치 환경에서는 이 명령을 실행할 수 없습니다.

Jetson first-run plan:

1. ONNX model과 대응되는 compiled TensorRT engine artifact를 준비합니다.
2. `--engine tensorrt --engine-path ...` 옵션으로 EdgeBench를 실행합니다.
3. engine deserialization, binding metadata extraction, CUDA buffer allocation, execution이 모두 통과하는지 확인합니다.
4. profiling 결과가 `results/*.json` 으로 저장되고 `engine=tensorrt`, `device=gpu`, `precision=fp16` 로 기록되는지 확인합니다.
5. 기존 result/report schema를 유지한 채 compare workflow까지 연결합니다.
...
edgebench compare-latest --model resnet18.onnx --engine tensorrt --device gpu --precision fp16
edgebench compare-latest --model yolov8n.onnx --engine tensorrt --device gpu --precision fp16
