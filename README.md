...
이 결과는 Odroid M2 실기 환경에서 RKNN runtime profiling 결과가 structured result로 저장되고,
같은 fp16 조건에서 `compare-latest`와 `history-report` 흐름에 재사용될 수 있음을 보여줍니다.
또한 structured result JSON 원문에 `runtime_artifact_path`, `primary_input_name`, `resolved_input_shapes`가 함께 기록되어 실행 provenance 추적이 가능합니다.
...
Odroid M2 실기 환경에서 `yolov8n.onnx` + `yolov8n_fp16.rknn` 조합의 RKNN runtime profiling도 성공했습니다.

실제 검증에서는:
- `engine=rknn`, `device=npu`, `precision=fp16` metadata 저장
