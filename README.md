...
- 현재 경로는 실기 검증이 끝난 first working path이며, production-grade runtime polish는 아직 남아 있습니다.
...
RKNN 기반 NPU 환경에서도 실제 inference validation이 가능하도록 확장되었습니다.

검증 환경:

- Odroid M1 (RK3568 NPU)
- Odroid M2 (RK3588 NPU)
- RKNN Toolkit 기반 모델 변환 및 실행

검증 완료 범위:

- YOLOv8n / YOLOv8s detection 모델
- FP16 vs INT8 (hybrid quantization) 비교
- 실제 NPU inference latency 측정
- detection accuracy (map50, F1 score) 결과 수집
- structured result 저장 및 compare 재사용

지원 기능:

- RKNN benchmark 결과 → EdgeBench result schema로 import
- cross-precision compare (fp16 vs int8)
...
EdgeBench는 단순 ONNX benchmark 도구를 넘어서  
**실제 Edge NPU 환경에서의 inference behavior validation까지 지원하는 시스템으로 확장되었습니다.**
...
> 전체 히스토리(raw)와 curated hardware validation 결과는 `BENCHMARKS.md` 참고
