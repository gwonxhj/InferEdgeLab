...
특히 Jetson TensorRT 경로에서는 단순 실행 성공만 확인한 것이 아니라,
실제 profiling 결과가 `structured result`로 저장되고,
동일 결과가 `compare-latest` 및 Markdown / HTML report 흐름으로 다시 연결되는지까지 검증했습니다.

### Jetson TensorRT (GPU)

- `resnet18`, `yolov8n` TensorRT profiling 실기 검증 완료
- `engine=tensorrt`, `device=gpu`, `precision=fp16` metadata 저장 확인
- `runtime_artifact_path`, `primary_input_name`, `resolved_input_shapes` 등 runtime provenance 저장 확인
- TensorRT execution → structured result → `compare-latest` → Markdown / HTML report pipeline 연결 확인

**Example (Jetson TensorRT, resnet18)**

| Run | Mean Latency | P99 Latency | Interpretation |
|---|---:|---:|---|
| Base | 3.4644 ms | 3.5057 ms | 기준 측정 |
| New | 3.6311 ms | 3.7170 ms | same-precision regression 감지 |

이 결과는 TensorRT 경로가 단순 실행 가능 여부만이 아니라,
동일 precision 조건에서의 regression tracking에도 재사용될 수 있음을 보여줍니다.
...
그 결과를 Markdown / HTML report로도 저장하여 report 재사용 경로까지 함께 확인했습니다.
